    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          {isOwner ? 'Resumen general de tu gimnasio.' : 'Resumen operativo del día.'}
        </p>
      </div>

      {/* Tarjetas principales */}
      <div className={`grid gap-4 ${isOwner ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-2 lg:grid-cols-3'}`}>
        <Card>
        ...
        {isOwner && (
          <Card>
            ...
            <CardContent>
              <p className="text-3xl font-bold">{formatPrice(data.monthIncome ?? 0)}</p>
            </CardContent>
          </Card>
        )}
      </div>

      {/* Segunda fila — solo owner */}
      {isOwner && (
        <div className="grid grid-cols-2 gap-4">
          <Card>
            ...
            <CardContent>
              <p className="text-3xl font-bold">{formatPrice(data.todayIncome ?? 0)}</p>
              <p className="text-sm text-muted-foreground mt-1">
                {data.todayPaymentsCount ?? 0} cobros
              </p>
            </CardContent>
          </Card>
      ...
      <div className={`grid grid-cols-1 gap-6 ${isOwner ? 'lg:grid-cols-2' : ''}`}>
        {/* Últimos pagos — solo owner */}
        {isOwner && data.recentPayments && (
          <Card>
            <CardHeader>
